...
const CampaignDetails = () => {
  ...
  const [selectedProspect, setSelectedProspect] =
    useState<ProspectWithEmail | null>(null);
  ...
  const [mailjetKeys, setMailjetKeys] = useState<{
    api_key: string;
    secret_key: string;
  } | null>(null);
  const [prospectLoadingStates, setProspectLoadingStates] = useState<
  ...
  const [deleteProspectDialogOpen, setDeleteProspectDialogOpen] =
    useState(false);
    ...
    try {
      ...
      if (prospectIds.length > 0) {
        ...
        if (emailsError) {
          ...
          emailsData = emails || [];
          console.log(
            `Fetched ${emailsData.length} emails for ${prospectIds.length} prospects`
          );
        }
      }

      const prospectsWithEmails: ProspectWithEmail[] = (
        prospectsData || []
      ).map((prospect) => {
        const email = emailsData.find(
          (email) => email.prospect_id === prospect.id
        );
        return {
          ...prospect,
          email_data: email || null,
  ...
  const generateEmailForProspect = async (
    ...
    try {
      ...
      const generatingToast = toast.loading(
        `${regenerate ? "Regenerating" : "Generating"} email for ${
          prospect.name
        }...`
      );
      ...
      const generationData = {
        ...
        company_description:
          campaign.company_description || "No description provided.",
      ...
      toast.success(
        `Email ${regenerate ? "regenerated" : "generated"} for ${prospect.name}`
      );
      ...
      if (generatedEmail.email_record) {
        ...
        if (error) {
          ...
          console.warn(
            "No email record returned from API or found in database, creating one manually"
          );
  ...
  const approveEmail = async () => {
    ...
    try {
      ...
      setProspects(
        prospects.map((p) =>
          p.id === selectedProspect.id
            ? { ...p, email_data: emailData as Email }
            : p
        )
      );
      ...
      const sendingToast = toast.loading(
        `Sending email to ${selectedProspect.name}...`
      );
  ...
  const rejectEmail = async () => {
    ...
    return (
      <div className="min-h-screen flex flex-col">
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          This action will permanently delete the campaign and
                          all associated prospects and emails. This action
                          cannot be undone.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                ...
                <div>
                  ...
                  <div className="mt-3 space-y-2">
                    <p className="text-sm text-gray-900">
                      ...
                      {campaign.company_name || "Not specified"}
                    </p>
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">Company Description:</span>{" "}
                      {campaign.company_description || "Not specified"}
                    </p>
                  </div>
                </div>
                <div>
                  ...
                  <div className="mt-3 space-y-2">
                    <p className="text-sm text-gray-900">
                      ...
                      {campaign.representative_name || "Not specified"}
                    </p>
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">Role:</span>{" "}
                      {campaign.representative_role || "Not specified"}
                    </p>
                    <p className="text-sm text-gray-900">
                      <span className="font-medium">Email:</span>{" "}
                      {campaign.representative_email || "Not specified"}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="bg-white shadow rounded-lg">
                  ...
                  {prospects.length > 0 ? (
                                ...
                                {prospect.email_data ? (
                                  <div className="flex items-center">
                                    <span
                                      ...
                                      ${
                                        prospect.email_data.status === "draft"
                                          ? "bg-yellow-100 text-yellow-800"
                                          : prospect.email_data.status ===
                                            "approved"
                                    ...
                                    >
                                      {prospect.email_data.status
                                        .charAt(0)
                                        .toUpperCase() +
                                        prospect.email_data.status.slice(1)}
                                    </span>
                                  </div>
                                ) : (
                              ...
                              <TableCell>
                                <div className="flex space-x-2">
                                  <Button
                                    ...
                                    size="sm"
                                    onClick={() =>
                                      generateEmailForProspect(prospect)
                                    }
                                    disabled={
                                  ...
                                  {prospect.email_data && (
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        generateEmailForProspect(prospect, true)
                                      }
                                      disabled={
                                        prospectLoadingStates[prospect.id] ||
                                        isGeneratingEmail
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="mt-1 text-sm text-gray-500">
                        Get started by adding your first prospect or importing a
                        CSV file.
                      </p>
        ...
        <AlertDialog
          open={deleteProspectDialogOpen}
          onOpenChange={setDeleteProspectDialogOpen}
        >
          <AlertDialogContent>
            <AlertDialogHeader>
              <AlertDialogTitle>Delete Prospect</AlertDialogTitle>
              <AlertDialogDescription>
                Are you sure you want to delete this prospect? This will also
                remove any associated emails and cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel
